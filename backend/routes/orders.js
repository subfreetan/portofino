...
// POST /orders
// Body carte      : { items: [{name, detail, qty, unitPrice}], paymentMethod: 'card' }
// Body comptoir   : { items: [...], paymentMethod: 'counter' }  — pas de paiement en ligne, réglé à la remise
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (!['card', 'counter', 'reward'].includes(paymentMethod)) {
    ...
    // ---------- Commande payée au comptoir (à la remise, aucun paiement en ligne) ----------
    if (paymentMethod === 'counter') {
      ...
      const { error: userErr } = await supabase
        .from('users')
        .update({
          points_balance: req.user.points_balance + pointsEarned,
          lifetime_points: req.user.lifetime_points + pointsEarned,
        })
        .eq('id', req.user.id);
